...
from script.data_config import OPPORTUNITY_SQL_TABLE, USER_SQL_TABLE, \
    ACCOUNT_SQL_TABLE, QUOTE_DETAIL_API_NAME, \
    QUOTE_DETAIL_TABLE_STRING, QUOTE_DETAIL_DICT, QUOTE_DETAIL_SQL_TABLE, QUOTE_DETAIL_CLOUMNS_ORDER, PRODUCT_SQL_TABLE, \
    QUOTE_SQL_TABLE
...
class Order_Data():
    def __init__(self):
        ...
        self.access_url  = ''
        self.binding = ''
        self.cloudcc_object=QUOTE_DETAIL_API_NAME
        self.sql_table= QUOTE_DETAIL_SQL_TABLE
        ...
        self.quote_table = QUOTE_SQL_TABLE
        ...
        self.sql_index_list=[]
        self.process_num = 1
        self.columns_order = QUOTE_DETAIL_CLOUMNS_ORDER
    ...
    def get_cloudcc_order(self,index):
        ...
        if data:
            ...
            local_quote_sql = """ select id as local_quote_id,crm_id as quote_id  from `{}` where crm_id in ({})""".format(self.quote_table,cc_product_str)
            local_quote_df = pd.read_sql_query(local_quote_sql,new_data)
            cc_df = pd.merge(cc_df, local_quote_df, how='left', on="quote_id")
            cc_df = cc_df.drop(["quote_id"],axis=1)
            cc_df = cc_df.rename(columns={"local_quote_id":"quote_id"})
            # # product_id
