...
from script.data_config import USER_SQL_TABLE, \
    ...
    ACTIVITY_TABLE_STRING, ACTIVITY_DICT, ACTIVITY_API_NAME, ACTIVITY_SQL_TABLE, OPPORTUNITY_SQL_TABLE, \
    ACTIVITY_CLOUMNS_ORDER
...
class Order_Data():
    def __init__(self):
        ...
        self.access_url  = ''
        self.binding = ''
        self.cloudcc_object=ACTIVITY_API_NAME
        self.sql_table= ACTIVITY_SQL_TABLE
        # self.sql_table= "activity_copy1"
        self.sql_mapping= ACTIVITY_DICT
        self.sql_table_string = ACTIVITY_TABLE_STRING
        ...
        self.sql_index_list=[]
        self.process_num = 1
        self.columns_order = ACTIVITY_CLOUMNS_ORDER
    ...
    def get_cloudcc_order(self,index):
        ...
        if data:
            ...
            # 处理object_id
            account_sql = """ select id as new_account_id, crm_id as account_id from {} """.format(self.account_table)
            ...
            for row in cc_df.itertuples():
                ...
                if isinstance(id,str):
                    ...
                    id = create_id(remark, timestamp, id_index)
