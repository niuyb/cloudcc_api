...
class Order_Data():
    def __init__(self):
        ...
        self.access_url  = ''
        self.binding = ''
        self.cloudcc_object=ORDER_DETAIL_API_NAME
        self.sql_table= ORDER_DETAIL_SQL_TABLE
        ...
        self.sql_index_list=[]
        self.process_num = 1
        self.columns_order = ORDER_DETAIL_CLOUMNS_ORDER
    ...
    def get_cloudcc_order(self,index):
        ...
        if data:
            ...
            for row in cc_df.itertuples():
                ...
                server_start = getattr(row, 'server_start')
                try:
                    cc_df.at[df_index, 'server_start'] = time_ms(server_start)
                except:
                    pass
                server_end = getattr(row, 'server_end')
                try:
                    cc_df.at[df_index, 'server_end'] = time_ms(server_end)
                except:
                    pass
            ...
            local_order_sql = """ select id as local_order_id,crm_id as order_id,contract_back_date,contract_status  from `{}` where crm_id in ({}) """.format(self.order_table,cc_order_str)
