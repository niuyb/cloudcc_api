...
class Order_Data():
    def __init__(self):
        ...
        self.access_url  = ''
        self.binding = ''
        self.cloudcc_object=PAYMENT_RECORD_API_NAME
        self.sql_table= PAYMENT_RECORD_SQL_TABLE
        # self.sql_table= "payment_record_copy1"
        self.sql_mapping= PAYMENT_RECORD_DICT
        self.sql_table_string = PAYMENT_RECORD_TABLE_STRING
        ...
        # self.today = "2021-03-01"
        self.today_stamp =  date_ms(self.today)
        print(self.today)
        self.one_times_num = 1000
        self.sql_index_list=[]
        self.process_num = 1
        self.columns_order = PAYMENT_RECORD_CLOUMNS_ORDER
    ...
    def get_cloudcc_order(self,index):
        ...
        if data:
            ...
            for row in cc_df.itertuples():
                ...
                actual_payment_date = getattr(row, 'actual_payment_date')
                try:
                    cc_df.at[df_index, 'actual_payment_date'] = time_ms(actual_payment_date)
                except:
                    pass
                back_date = getattr(row, 'back_date')
                try:
                    cc_df.at[df_index, 'back_date'] = time_ms(back_date)
                except:
                    pass
            ...
            local_department_df = local_split_df[["owner_id","department"]]
            # department
            cc_df = pd.merge(cc_df, local_department_df, how='left', on="owner_id")
            #owner_id
            ...
            # order_actual_owner
            local_user_df = local_user_df.rename(columns={"updated_by": "order_actual_owner"})
            cc_df = pd.merge(cc_df, local_user_df, how='left', on="order_actual_owner")
            cc_df = cc_df.drop(["order_actual_owner"], axis=1)
            cc_df = cc_df.rename(columns={"local_owner_id": "order_actual_owner"})
            # order_owner
            local_user_df = local_user_df.rename(columns={"order_actual_owner": "order_owner"})
            cc_df = pd.merge(cc_df, local_user_df, how='left', on="order_owner")
            cc_df = cc_df.drop(["order_owner"], axis=1)
            cc_df = cc_df.rename(columns={"local_owner_id": "order_owner"})
